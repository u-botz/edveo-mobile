...
class AddSubjectBottomSheet extends ConsumerStatefulWidget {
  final int courseId;
  const AddSubjectBottomSheet({super.key, required this.courseId});
...
class _AddSubjectBottomSheetState extends ConsumerState<AddSubjectBottomSheet> {
  ...
  Future<void> _submit() async {
    ...
    final err = await ref.read(courseManageProvider(widget.courseId).notifier).addSubject(_nameCtrl.text.trim());
    if (!mounted) return;
    if (err == null) {
      ...
      child: Form(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('Add Subject', style: _pjs(size: 18, weight: FontWeight.w700)),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: _loading
                    ...
                    : Text('Add Subject', style: _pjs(size: 14, weight: FontWeight.w600, color: Colors.white)),
              ),
            ),
          ],
        ),
